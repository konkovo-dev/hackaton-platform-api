# SkillsService gRPC Guide

## Описание

`SkillsService` предоставляет API для работы с каталогом навыков.
...
grpcurl -plaintext \
  ...
  localhost:50051 identity.v1.SkillsService/ListSkillCatalog
```
...
Сохраните как `test-skills-service.sh`:
...
Запустите:
```bash
chmod +x test-skills-service.sh
./test-skills-service.sh
```
...
grpcurl -plaintext \
  -H "authorization: Bearer $ACCESS_TOKEN" \
  -d '{"query": {"page": {"page_size": 100}}}' \
  localhost:50051 identity.v1.SkillsService/ListSkillCatalog | jq '.skills[] | {id, name}'

# Посчитать количество навыков
grpcurl -plaintext \
  -H "authorization: Bearer $ACCESS_TOKEN" \
  -d '{"query": {"page": {"page_size": 100}}}' \
  localhost:50051 identity.v1.SkillsService/ListSkillCatalog | jq '.skills | length'
```
