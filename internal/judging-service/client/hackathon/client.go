...
func (c *Client) GetHackathon(ctx context.Context, hackathonID string) (stage string, resultPublishedAt *time.Time, err error) {
	...
	var resultPublished *time.Time
	if resp.Hackathon.ResultPublishedAt != nil {
		t := resp.Hackathon.ResultPublishedAt.AsTime()
		resultPublished = &t
	}

	return stageStr, resultPublished, nil
}
