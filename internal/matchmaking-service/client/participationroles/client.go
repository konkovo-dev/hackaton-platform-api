...
func (c *Client) GetParticipationAndRoles(ctx context.Context, userID, hackathonID string) (roles []string, teamID *string, err error) {
	...
	// Create fresh outgoing metadata for GetHackathonContext
	...
	ctxResp, err := c.staffService.GetHackathonContext(ctxWithMD, ctxReq)
	if err != nil {
	...
	if ctxResp.ParticipationStatus == participationandrolesv1.ParticipationStatus_PART_TEAM_MEMBER ||
		...
		// Create fresh outgoing metadata for GetUserParticipation
		partMD := metadata.New(map[string]string{
			"x-service-token": c.serviceToken,
		})
		if incomingMD, ok := metadata.FromIncomingContext(ctx); ok {
			if auth := incomingMD.Get("authorization"); len(auth) > 0 {
				partMD.Set("authorization", auth[0])
			}
		}
		partCtx := metadata.NewOutgoingContext(ctx, partMD)

		partResp, err := c.participationService.GetUserParticipation(partCtx, partReq)
