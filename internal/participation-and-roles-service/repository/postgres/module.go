		...
		func(lc fx.Lifecycle, cfg *pgxutil.Config, logger *slog.Logger) (*pgxpool.Pool, error) {
			...
			lc.Append(fx.Hook{
				OnStart: func(ctx context.Context) error {
					logger.Info("participation-and-roles-service database connection pool initialized")
					return nil
				},
				OnStop: func(ctx context.Context) error {
					logger.Info("closing participation-and-roles-service database connection pool")
					pool.Close()
					return nil
				},
			})
