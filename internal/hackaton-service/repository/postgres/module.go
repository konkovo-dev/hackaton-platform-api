...
var Module = fx.Module("postgres",
	fx.Provide(
		...
		NewHackathonRepository,
		NewHackathonLinkRepository,
		...
		func(lc fx.Lifecycle, cfg *pgxutil.Config, logger *slog.Logger) (*pgxpool.Pool, error) {
			...
			lc.Append(fx.Hook{
				OnStart: func(ctx context.Context) error {
					logger.Info("hackaton-service database connection pool initialized")
					return nil
				},
				OnStop: func(ctx context.Context) error {
					logger.Info("closing hackaton-service database connection pool")
					pool.Close()
					return nil
				},
			})
