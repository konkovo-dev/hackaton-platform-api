...
func (s *HackathonService) CreateHackathon(ctx context.Context, req *hackathonv1.CreateHackathonRequest) (*hackathonv1.CreateHackathonResponse, error) {
	...
	if idempotencyKey != "" {
		...
		found, err := s.idempotency.CheckAndGet(ctx, idempotencyKey, "create_hackathon", requestHash, resp)
		if err != nil {
	...
	in := mappers.ProtoToCreateHackathonIn(req)

	result, err := s.hackathonService.CreateHackathon(ctx, in)
	if err != nil {
	...
	if idempotencyKey != "" {
		requestHash := idempotency.ComputeHash(req.Name, req.ShortDescription)
		if err := s.idempotency.Save(ctx, idempotencyKey, "create_hackathon", requestHash, resp); err != nil {
			s.logger.ErrorContext(ctx, "failed to save idempotency key", slog.String("error", err.Error()))
		}
	}

	s.logger.InfoContext(ctx, "hackathon created", slog.String("hackathon_id", result.HackathonID.String()))
	return resp, nil
}
