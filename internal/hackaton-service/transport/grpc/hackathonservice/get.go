...
func (s *HackathonService) GetHackathon(ctx context.Context, req *hackathonv1.GetHackathonRequest) (*hackathonv1.GetHackathonResponse, error) {
	hackathonID, err := uuid.Parse(req.HackathonId)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "invalid hackathon_id: %v", err)
	}

	in := hackathon.GetHackathonIn{
	...
	if err != nil {
		return nil, s.handleError(ctx, err, "GetHackathon")
	}

	return mappers.GetHackathonOutToProto(out), nil
}
