		...
		func(lc fx.Lifecycle, cfg *pgxutil.Config, logger *slog.Logger) (*pgxpool.Pool, error) {
			...
			lc.Append(fx.Hook{
				OnStart: func(ctx context.Context) error {
					logger.Info("submission-service database connection pool initialized")
					return nil
				},
				OnStop: func(ctx context.Context) error {
					logger.Info("closing submission-service database connection pool")
					pool.Close()
					return nil
				},
			})
