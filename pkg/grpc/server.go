...
func NewServer(opts ...ServerOptions) *grpc.Server {
	...
	if len(opts) > 0 {
		if len(opts[0].UnaryInterceptors) > 0 {
			serverOpts = append(serverOpts,
				grpc.ChainUnaryInterceptor(opts[0].UnaryInterceptors...))
		}
		if len(opts[0].StreamInterceptors) > 0 {
			serverOpts = append(serverOpts,
				grpc.ChainStreamInterceptor(opts[0].StreamInterceptors...))
		}
	}
...
func RunServer(lc fx.Lifecycle, s *grpc.Server, lis net.Listener, logger *slog.Logger) {
	lc.Append(fx.Hook{
